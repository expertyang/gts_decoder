...
!   ENDIF
      ...
      CASE DEFAULT
         ...
         IF ( part .EQ. 'A' .OR. part .EQ. 'C' ) THEN
            ...
            ELSEIF ( arg(1:1) .EQ. '7' .OR. arg(1:1) .EQ. '6' ) THEN
               pressure = MISSING
               height = multiply ( 10 , str2int ( arg(2:5) ) )
               cval='....:'//arg(1:1)//'HHHH:Z @ max V (gpm)'
               CALL record_appendj ( minus(pressure) , height , cval )
               CALL new_section ( 3 )
               RETURN
   ...
   ck_sections : SELECT CASE ( section_id )
      ...
      CASE ( 42 )    ; ! skip this section CALL decode_fm32_sec42 ( arg )
...
SUBROUTINE decode_fm32_sec2 ( arg )
   ...
   INTEGER , DIMENSION(NLVL), PARAMETER :: std_lvls  = &
   ...
   ck_next_group : SELECT CASE ( section_subgrp )
      ...
      CASE ( 1 )
         ...
         ELSE
            ...
            ENDIF
            section_subgrp = -1
            find_std_lvl : DO npre = 1, NLVL
!               WRITE (*, *) npre, std_lvls ( npre ), pressure
               IF ( std_lvls ( npre ) .EQ. pressure ) THEN
                  CALL record_appendi ( pressure , cval )
                    
                  WRITE(*, *) "std group pressure:",pressure  ,npre 
      ...
      CASE ( 2 : 4 )
         ...
         WRITE(*, *) "next group pressure:",pressure  ,npre 
...
SUBROUTINE decode_fm32_sec42 ( arg )
   ...
   INTEGER                               :: ival , iget , jval
   CHARACTER ( LEN = rlen )              :: cval , cget
   CHARACTER ( LEN = llen )              :: arg1 

   IF ( TRACE_MOST ) PRINT *,'decode_fm32_sec42: in ', &
      section_subgrp, ' ', TRIM(arg)

   if(arg(1:1)/=arg(2:2))then

...
!  the following block is for fm-96 acars-type reports
   ELSE IF ( arg(1:1) .EQ. 'M' ) THEN
      ival = str2int ( TRIM(arg(2:)) )
      ival = minus ( ival )
      cval = arg

   ELSE IF ( arg(1:1) .EQ. 'P' ) THEN
      ival = str2int ( TRIM(arg(2:)) )
      cval = arg
! end of fm-96 format junk JFB 2/20/99
...
SUBROUTINE code_amdar_temp ( arg , ival , rval , cval , append ) ;
   USE record_def
   IMPLICIT NONE
   INCLUDE 'code_tables.fi'
   ...
   IF ( ival .eq. MISSING ) THEN
      CALL code_ignore ( 'amdar_temp', 'Missing Temperature', arg )
      cval = 'missing <' // TRIM(arg) // '>'
      rval = ival

   ELSE IF ( ival .LT. 0 ) THEN
      CALL code_error ( 'amdar_temp', 'Invalid Temperature', arg )
      cval = 'invalid <' // TRIM(arg) // '>'
      rval = ival
   ...
   cval = 'TEMP:AMDAR:Temperature (C)' // TRIM(cval)
   IF ( append .EQ. 'APPEND' ) CALL record_appendr ( ival , rval , cval )

ENDSUBROUTINE


SUBROUTINE code_arp_wind ( arg , ival , rval , cval , append ) ;
! This subroutine decodes the arp (acars) format wind. It has no '/'
! JFB 2/20/99
   USE record_def
   IMPLICIT NONE
   INCLUDE 'code_tables.fi'
...
SUBROUTINE code_airep_wind ( arg , ival , rval , cval , append ) ;
   ...
   IF ( ival .LT. 0 ) THEN
      ...
      section_id = -1
   ELSE
      if(center_id=="ZBBB")then ! unit is m/s
         rval = multiply ( ival, 1. )
      else
         rval = multiply ( ival, KNOT )
      endif
   ENDIF
...
SUBROUTINE code_table_icao ( arg , ival , rval , cval , append )
   ...
   IMPLICIT NONE
!  INCLUDE 'code_tables.fi'
! replace the INCLUDE code_tables.fi with the following lines
...
!  write(*, *) ival
!  IF ( ivalVERIFY ( TRIM(arg(1:4)) , NUMBER_SET ) .EQ. 0 ) THEN
!   IF ( VERIFY ( TRIM(arg(1:4)) , NUMBER_SET ) .EQ. 0 ) THEN
!    READ ( icao_data , stn_fmt , rec=ival ) stn_dscrptn
!   write(*, *) stn_dscrptn
!   ENDIF
   READ ( icao_idx , rec = ival ) index
   IF ( index > 0 ) THEN
      READ ( icao_data , stn_fmt , rec=index ) stn_dscrptn
      IF ( arg(1:4) .EQ. stn_dscrptn(1:4) ) THEN
         cur_sttn_id = str2int(stn_dscrptn(5:9))
!        WRITE(*, '(a)' ) stn_dscrptn
!        WRITE(*, '(a)' ) stn_dscrptn(10:15)
...
SUBROUTINE code_table_0200 ( arg , ival , rval , cval , append ) ;
   ...
   SELECT CASE ( ival )
      ...
      CASE DEFAULT
         ...
         ELSE
            rval = UNDEFINED
            cval = 'invalid tendency <' // arg // '>'
            CALL code_error ( '0200', 'Pressure Tendency', arg )
         ENDIF
   ENDSELECT
   cval = '0200:appp :Pre. Tendency ' // cval
   IF ( append .EQ. 'APPEND' ) CALL record_appendr ( ival , rval , cval )
ENDSUBROUTINE
...
SUBROUTINE code_table_0515 ( arg , ival , rval , cval , append )
   ...
   SELECT CASE ( arg )
      ...
      CASE DEFAULT
         rval = UNDEFINED
         cval = 'Middle clouds invalid <' // arg // '>'
...
!      rval = multiply ( iget , 0.1 * TOMETER )
      rval = multiply ( iget , 0.1 )
   ELSE
!      rval = multiply ( iget , TOMETER )
      rval = iget
   ENDIF
   ...
   ival = MISSING
   SELECT CASE ( iget )
      CASE ( 0 ) ;       cval = '(dt nostandard) <' // arg // '>'
      CASE ( MISSING ) ; cval = '(dt invalid) <' // arg // '>'
!      CASE ( 1 : 9 ) ;   cval = cc(iget) // ' hrs preceding obs. (m)'
      CASE ( 1 : 9 ) ;   cval = cc(iget) // ' hrs preceding obs. (mm)'
...
! Make the time window one hour long  jfb  2/20/99
...
SUBROUTINE code_table_PPhhh ( arg, ival, rval, cval, append, pressure, part )
   ...
   IF ( rval .EQ. UNDEFINED ) THEN
      cval = TRIM(cval) // ' invalid <' // arg // '>' // BLANK_LINE
   ELSEIF ( rval .EQ. MISSING ) THEN
      cval = TRIM(cval) // ' Missing <' // arg // '>' // BLANK_LINE
   ENDIF
   IF ( append .EQ. 'APPEND' ) CALL record_appendr ( pressure , rval , cval )
ENDSUBROUTINE
...
!      CASE ( 9998 )        ; rval = TOMETER * 0.1 * ival ; cval = '>= 0.9998 m'
      CASE ( 0000 : 9997 ) ; rval = 0.1 * ival
      CASE ( 9998 )        ; rval = 0.1 * ival ; cval = '>= 999.8 mm'
...
SUBROUTINE code_table_SATID ( arg , ival , rval , cval , append )
   ...
   IF ( arg(1:1) .EQ. '2' ) THEN
      ...
      SELECT CASE ( str2int ( arg(2:3) ) )
         ...
         CASE DEFAULT           ; cval = 'US unknown ### <' // arg // '>'
      ENDSELECT
...
SUBROUTINE code_table_stnid ( arg , ival , rval , cval , append )
   ...
   ELSE
      READ ( stn_idx , rec=ival ) index
      IF ( index > 0 ) THEN
      READ ( stn_data , stn_fmt , rec = index ) stn_dscrptn
!     READ ( stn_data, rec = index ) stn_dscrptn
!      READ ( stn_data , stn_fmt , rec = ival ) stn_dscrptn
!      IF ( ival .EQ. str2int( stn_dscrptn(1:5) ) ) THEN
!        WRITE(*, *) stn_dscrptn
!        WRITE(*, *) stn_dscrptn( 7:11)
!        WRITE(*, *) stn_dscrptn(14:19)
!        WRITE(*, *) stn_dscrptn(22:26)
         cur_sttn_id = ival
!        iget = str2int ( stn_dscrptn( 7:11) )
         read(stn_dscrptn( 7:11) ,*) iget
         rget = FLOAT( iget )/ 100.
         cval = '....:' // arg(1:5) // ':Latitude (N) WMO ' // stn_dscrptn(31:)
         CALL record_appendi ( iget , cval )
!        iget = str2int ( stn_dscrptn(14:19) )
         read(stn_dscrptn(14:19) ,*) iget
         rget = FLOAT( iget )/ 100.
         cval = '....:' // arg(1:5) // ':Longitude(E) WMO ' // stn_dscrptn(31:)
         CALL record_appendi ( iget , cval )
!        iget = str2int ( stn_dscrptn(22:26) )
         read(stn_dscrptn(22:26) ,*) iget
         If (iget == 9999) iget = MISSING
         cur_sttn_alt = iget
...
!
! Standard Pre. (hPa)  925 850  700  500  400  300  250  200  150  100
! Approx. Height (km)  .85 1.5  3.1  5.8  7.6  9.5 10.6 12.3 14.1 16.6
!
! Standard Pre. (hPa)  100   70   50   30   20   10 .... //   7    5    3    2    1 .... 
! Approx. Height (km) 16.6 18.5 20.5 24.0 26.5 31.0 .... //33.5 36.0 39.0 42.0 48.0 .... 
!
! *********** This need to be revisited ****************************
   ...
   INTEGER, DIMENSION(NLVL), PARAMETER :: gpm = (/    0,        &
        ...
        14100, 16600, 18500, 20500, 24000, 26500, 31000 , 33500, 36000, 39000, 42000, 48000 /)
...
SUBROUTINE code_error ( routine_name , description , value )
   ...
   ! Ôsort of?generic error handling routine 1, see also code_warn
   !
   ! Created : May 22, 1995    Alexis Lau (HKUST/NCAR)
...
SUBROUTINE code_warn ( routine_name , description , value )
   ...
   ! Ôsort of?generic error handling routine 2, see also code_error
   !
   ! Created : May 22, 1995    Alexis Lau (HKUST/NCAR)

...
SUBROUTINE decode_fm12_sec1 ( arg )
   ...
   IF ( section_argnum .EQ. 2 ) THEN
      ! Check Nddff Missing 
      if(arg(1:1)=="1")then
         call get_next_word_in_mesg ( next_arg )
         if(next_arg(1:1)/="1")then ! This is Not Nddff Group
            section_argnum = 3 
         endif
         call push_word_back_to_mesg ( next_arg )
      endif
   ENDIF
   ...
   ELSE
      ...
      CASE ( '7' )
         IF ( sttn_op .EQ. 1 .OR. sttn_op .EQ. 4 ) THEN
...
!                            'ASIA:TgTg :Ground / Grass Temperature (ASIA) ' )
         IF (region_AA=='CI') THEN !CHINA 0P24P24T24T24 
            ...
            IF (rval >= 50) THEN
                ...
                rval = minus(rval)
            ENDIF
            CALL record_appendr ( ival , rval , '....:T24T :24-h Temp. Tendency' )
         ELSE
            CALL code_ignore ( 'fm12_sec3' , '0....' , arg )
         ENDIF
      CASE ( '1' )
         CALL code_table_3845 ( arg(2:2) , ival , rval , cval , ' ' )
         rval = multiply ( multiply ( 0.1 , rval ) , str2int ( arg(3:5) ) )
...
!         IF(region=='Asia')THEN
      ...
      CASE ( '8' )
         ...
         CALL record_appendr ( iget , rget , cget )
!         WRITE(*, *)  'sec3, argnum8 ', iget , rget , cget 

...
!     CALL bulletin_skip ( ETX )
   ...
   loop_mesgs : DO WHILE ( ASSOCIATED ( current_line ) )
      ...
      loop_words : DO
         ...
         ELSE
            CALL decode_airep_words ( arg , ifm96 )  ! decode the word
...
SUBROUTINE decode_airep_words ( arg , ifm96 )

   IMPLICIT NONE
   CHARACTER ( LEN = * ) , INTENT ( IN ) :: arg
   INTEGER , INTENT ( IN)                :: ifm96  ! jfb
   REAL                                  :: rval , rget
   INTEGER                               :: ival , iget
   ...
   CALL decode_airep_sec1 ( arg , ifm96 )
...
SUBROUTINE decode_airep_sec1 ( arg , ifm96 )
...
! jfb mods
      if ( ifm96 .eq. 0 .or. SCAN ( TRIM(arg) , '/' ) .ne. 0 ) then
        CALL code_airep_wind ( arg, ival, rval, cval, 'APPEND' )
      else
        CALL code_arp_wind ( arg, ival, rval, cval, 'APPEND' )
      endif
      section_argnum = 7
